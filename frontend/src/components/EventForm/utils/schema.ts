...
const schema = {
	...
	organizerEmail: z
		...
		.max(128, { message: 'Email should not exceed 128 characters' }),
	eventName: z
		.string()
		.min(1, { message: 'Please enter the event name' })
		.max(50, { message: 'Event name should not exceed 50 characters' }),
	date: z
	...
	time: z
		.date({ required_error: 'Please select a time', invalid_type_error: 'Please select a time' })
		.refine(time => time instanceof Date, { message: 'Please enter a valid time' }),
	timezone: z.object({
		value: z.string(),
		label: z.string(),
	}),
